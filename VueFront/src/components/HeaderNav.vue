<template>
  <div class="header">
    ...
    >
      ...
      <div class="position-absolute float-right userinfo justify-content-end" v-if="userInfo">
        <div>{{ userInfo.username }}({{ userInfo.userid }})님 환영합니다.</div>
        <a class="p-0 m-0 display-block" @click.prevent="onClickLogout">로그아웃</a>
      </div>
    </nav>
  </div>
</template>
