    ...
    <b-container>
      <div class="row g-4">
        <div class="col-lg-12 text-center">
          ...
          <button class="btn btn-dark" @click="randomSearchTD" v-show="restartTD">
            다른 관광지 보기
          </button>
        </div>
...
export default {
  ...
  created() {
    ...
    http.get(`/traffic/gettd`).then(({ data }) => {
  ...
  methods: {
    ...
    randomSearchTD() {
      ...
      http.get(`/traffic/gettddetail/${this.td_info.tourSeq}`).then(({ data }) => {
        ...
        if (data.msgBody.climbingRoadDesc == null)
          this.td_info_detail.climbingRoadDesc = "정보 없음";
